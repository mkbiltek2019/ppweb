angular.module('app', ['ui.router', 'pascalprecht.translate', 'chart.js', 'ngStorage', 'functions', 'charts']).config(['$stateProvider', '$urlRouterProvider', '$translateProvider', '$translatePartialLoaderProvider', '$httpProvider', function ($stateProvider, $urlRouterProvider, $translateProvider, $translatePartialLoaderProvider, $httpProvider) {
    ...
    $scope.today = new Date(); function initPrintSettings() { $http({ url: $sessionStorage.config.backend + 'PrintPdf.asmx/InitMenuSettings', method: "POST", data: {} }).then(function (response) { $scope.settings = JSON.parse(response.data.d) }, function (response) { alert(response.data.d) }) }; var saveVersion = function () {
    ...
    var getLanguageTitle = function (x) { if ($scope.config !== undefined) { angular.forEach($scope.config.languages, function (value, key) { if (value.code == x) { $scope.currLanguageTitle = value.title; return !1 } }) } }
    $scope.setLanguage = function (x) {
        ...
        if (x == null || x == '' || x == 'null') { alert($translate.instant('enter the activation code')); return !1 }
    ...
    var loadPals = function () { $http({ url: $sessionStorage.config.backend + 'Calculations.asmx/LoadPal', method: "POST", data: '' }).then(function (response) { $scope.pals = JSON.parse(response.data.d) }, function (response) { alert(response.data.d) }) }; var getConfig = function () {
        $scope.userId = null; $scope.clientId = null; $http.get('./config/config.json').then(function (response) {
            ...
            if ($scope.userId == null || $scope.clientId == null) { $state.go('login'); return !1 }
        ...
        if (angular.isUndefined($rootScope.clientData)) { return !1 }
        var x = $scope.clientData; if (x.clientId != null && x.height > 0 && x.weight > 0 && x.pal.value > 0) { return !0 } else { return !1 }
    }
    ...
    var getCalculation = function () {
        if (isNaN($scope.clientData.weight) == !0 || isNaN($scope.clientData.height) == !0 || isNaN($scope.clientData.waist) == !0 || isNaN($scope.clientData.hip) == !0) { return !1 }
    ...
    getConfig(); $scope.loading = !1; $scope.loadMenues = function () { $scope.loading = !0; $http({ url: $sessionStorage.config.backend + 'Menues.asmx/LoadClientMenues', method: "POST", data: { userId: $scope.userId, clientId: $scope.clientId } }).then(function (response) { $scope.menus = JSON.parse(response.data.d); angular.forEach($scope.menus, function (x, key) { var date_ = new Date(x.date); x.date = date_.toLocaleDateString() }); $scope.loading = !1 }, function (response) { $scope.loading = !1; alert(response.data.d) }) }
    ...
    var consumers = 1; $scope.pdfLink = null; $scope.creatingPdf = !1; $scope.createMenuPdf = function () { $scope.pdfLink = null; $scope.creatingPdf = !0; $http({ url: $sessionStorage.config.backend + 'PrintPdf.asmx/MenuPdf', method: "POST", data: { userId: $scope.userId, currentMenu: $scope.menu, totals: $scope.totals, consumers: consumers, lang: $scope.config.language, settings: $scope.settings, date: null, author: null, headerInfo: null } }).then(function (response) { var fileName = response.data.d; $scope.creatingPdf = !1; $scope.pdfLink = $sessionStorage.config.backend + 'upload/users/' + $scope.userId + '/pdf/' + fileName + '.pdf' }, function (response) { $scope.creatingPdf = !1; alert(response.data.d) }) }
    ...
    $scope.show = !1; $scope.showTitle = 'show'; $scope.showOther = function () { $scope.show = !$scope.show; if ($scope.show == !0) { $scope.showTitle = 'hide' } else { $scope.showTitle = 'show' } }
    $scope.showLog = !0; $scope.showClientLog = function () { $scope.showLog = !$scope.showLog }
    ...
    var setClientLogGraphData = function (type, clientLogsDays) {
        $scope.clientLog_ = []; var clientLog = []; var labels = []; if (!angular.isDefined($scope.calculation)) { return !1 }
        if (angular.isDefined($scope.calculation.recommendedWeight)) {
            ...
            angular.forEach($scope.clientLog, function (x, key) { if (functions.getDateDiff(x.date) <= days) { $scope.clientLog_.push(x); clientLog.push(x.weight); if (key % (Math.floor($scope.clientLog.length / 31) + 1) === 0) { labels.push(new Date(x.date).toLocaleDateString()) } else { labels.push("") } } }); $scope.clientLogGraphData = charts.createGraph([$translate.instant("")], [clientLog], labels, ['#3399ff'], { responsive: !0, maintainAspectRatio: !1, legend: { display: !0 }, scales: { xAxes: [{ display: !0, scaleLabel: { display: !0 }, ticks: { beginAtZero: !1 } }], yAxes: [{ display: !0, scaleLabel: { display: !0 }, ticks: { beginAtZero: !0 } }] } }, [{ label: $translate.instant("weight") + ' (' + $translate.instant("kg") + ')', borderWidth: 3, type: 'line', fill: !0 }])
        }
    }; getClientData()
    ...
    google.charts.load('current', { 'packages': ['gauge'] }); var bmiChart = function () {
        if (!angular.isDefined($scope.calculation)) { return !1 }
    ...
    var setClientLogGraphData = function (type, clientLogsDays) {
        $scope.clientLog_ = []; var clientLog = []; var goalFrom = []; var goalTo = []; var goalWeight = []; var labels = []; if (!angular.isDefined($scope.calculation)) { return !1 }
        if (angular.isDefined($scope.calculation.recommendedWeight)) {
            var days = 30; var goal = 0; var deficit = ($scope.calculation.recommendedEnergyIntake - $scope.calculation.recommendedEnergyExpenditure) - $scope.calculation.tee; if (clientLogsDays !== undefined) { days = clientLogsDays.days; $scope.clientLogsDays = clientLogsDays }
        ...
        $scope.clientLogGraphData = charts.createGraph([$translate.instant("measured value"), $translate.instant("lower limit"), $translate.instant("upper limit"), $translate.instant("goal")], [clientLog, goalFrom, goalTo, goalWeight], labels, ['#3399ff', '#ff3333', '#33ff33', '#ffd633'], { responsive: !0, maintainAspectRatio: !1, legend: { display: !0 }, scales: { xAxes: [{ display: !0, scaleLabel: { display: !0 }, ticks: { beginAtZero: !1 } }], yAxes: [{ display: !0, scaleLabel: { display: !0 }, ticks: { beginAtZero: !1 } }] } }, [{ label: $translate.instant("measured value"), borderWidth: 1, type: 'bar', fill: !0 }, { label: $translate.instant("lower limit"), borderWidth: 2, type: 'line', fill: !1 }, { label: $translate.instant("upper limit"), borderWidth: 2, type: 'line', fill: !1 }, { label: $translate.instant("goal") + ' (2 ' + $translate.instant("kg") + '/' + $translate.instant("mo") + ')', borderWidth: 3, type: 'line', fill: !1 }])
    }; $scope.setClientLogGraphData = function (type, clientLogsDays) { setClientLogGraphData(type, clientLogsDays) }
    $scope.removeClientLog = function (x) { var r = confirm($translate.instant('delete record') + '?'); if (r == !0) { removeClientLog(x) } }
    ...
    return {
        restrict: 'A', link: function (scope, elm, attrs, ctrl) {
            elm.on('keydown', function (event) {
                var $input = $(this); var value = $input.val(); value = value.replace(',', '.'); $input.val(value); if (event.which == 64 || event.which == 16) { return !1 } else if (event.which >= 48 && event.which <= 57) { return !0 } else if (event.which >= 96 && event.which <= 105) { return !0 } else if ([8, 13, 27, 37, 38, 39, 40].indexOf(event.which) > -1) { return !0 }
                else if (event.which == 110 || event.which == 188 || event.which == 190) { return !0 } else if (event.which == 46) { return !0 }
                else { event.preventDefault(); return !1 }
            })
        }
    }
})